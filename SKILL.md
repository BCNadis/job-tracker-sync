...
description: Weekly sync of a Job Application Tracker Google Sheet from your Gmail.
---

# Job Tracker Sync

Keep your "Job Application Tracker" Google Sheet current from your Gmail. This skill is fully self-contained — run it on demand or on a schedule (e.g. Mondays 8am).

## Configuration — edit before first run

Replace the placeholder below with the ID of YOUR sheet. The file ID is the long
string in your sheet's URL between `/d/` and `/edit`
...
Do not search for a different file — always use the configured one above.
...
Skip newsletters, job-board digests (LinkedIn/Indeed/etc. "jobs for you"), and automated marketing. Only keep genuine messages about a specific application of yours.
...
STEP 4 — Reconcile against the sheet:
  ...
  - ALREADY in the table → update Status and Last Update ONLY. NEVER overwrite Date Applied, Progress (unless empty), Notes, or Source. The Notes column is yours — never touch it.
...
The Google Drive connector is read-only for cell edits, so edit via the Claude in Chrome browser extension:
  - First check a browser is connected (list_connected_browsers). If none is connected, DO NOT guess — skip writing, and in the summary clearly state that the changes could not be written because no browser was connected, and list the exact rows/updates that are pending so you can paste them or connect Chrome.
...
Notification: notify the user on completion with the summary.
